...
. link:#production-deployment[Production Deployment]
. link:#ionos-deploy-now[IONOS Deploy Now]
. link:#docker-deployment[Docker Deployment]
. link:#kubernetes-deployment[Kubernetes Deployment]
. link:#security-hardening[Security Hardening]
. link:#monitoring-setup[Monitoring Setup]
. link:#backup-and-recovery[Backup and Recovery]
...
*Minimum (Development)* - 4 CPU cores - 8 GB RAM - 50 GB storage - 100
Mbps network

*Recommended (Production)* - 8 CPU cores - 16 GB RAM - 200 GB SSD
storage - 1 Gbps network - UPS backup power
...
. Select the Kaldor-IIoT repository
...
. Import from `+infrastructure/monitoring/grafana/dashboards/+`
...
For deployment issues: - Email: devops@kaldor-iiot.example.com -
